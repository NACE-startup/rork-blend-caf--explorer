...
export default publicProcedure
  ...
  .query(async ({ input }) => {
    const allCafes = Array.from(db.cafes.values());
    const cafes = allCafes.slice(input.offset, input.offset + input.limit);
    ...
    return {
      ...
      hasMore: input.offset + input.limit < allCafes.length,
    };
  });
